...
export default function PublicCalendarPage() {
  ...
  const openAdd = () => {
    setModal({ mode: "add", data: { ...EMPTY_FORM, branch_name: branch !== "전체" ? branch : "철산점", start_datetime: selectedDate + "T10:00" } });
  };
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .public-cal .react-calendar { width: 100%; border: none; background: transparent; font-family: -apple-system, 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif; }
        .public-cal .react-calendar__navigation { display: flex; align-items: center; margin-bottom: 12px; gap: 4px; }
        .public-cal .react-calendar__navigation button { background: none; border: none; font-size: 15px; font-weight: 700; color: #1e293b; cursor: pointer; padding: 6px 10px; border-radius: 8px; min-width: 36px; }
        ...
        .public-cal .react-calendar__tile { background: none; border: none; border-radius: 12px; padding: 8px 4px; cursor: pointer; font-size: 14px; font-weight: 600; color: #1e293b; transition: background 0.15s; min-height: 72px; display: flex; flex-direction: column; align-items: center; }
        ...
        .inp { width: 100%; border: 1.5px solid #e2e8f0; border-radius: 10px; padding: 9px 13px; font-size: 14px; background: #fff; outline: none; font-family: inherit; color: #0f172a; }
        ...
        .btn-sec { background: #f1f5f9; color: #334155; border: none; border-radius: 10px; padding: 9px 20px; font-size: 13px; font-weight: 700; cursor: pointer; }
      `}</style>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: "#94a3b8", marginTop: 4 }}>{selectedRows.length > 0 ? `총 ${selectedRows.length}건 예약` : "예약 없음"}</div>
              </div>
            ...
            {selectedRows.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {selectedRows.map((r) => {
                  ...
                  return (
                    <div key={r.event_id} onClick={() => openEdit(r)} style={{
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 15, fontWeight: 700, color: "#1e293b" }}>
                            {naver && <span style={{ fontSize: 10, marginRight: 3 }}>🔒</span>}
                            {r.customer_name || "-"}
                          </span>
                        </div>
